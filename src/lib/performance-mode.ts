...
export const IS_DATA_DIRECTORY_ISOLATED = IS_DEVELOPMENT_BUILD || IS_PERFORMANCE_BUILD;
/**
 * Development and Performance Lab builds only. Keyed off the Vite *mode* (not
...
/**
 * Runs before providers mount so an isolated build cannot inherit or retain a
 * path to a real library and is visually obvious from its first frame.
 */
export function prepareIsolatedBuildEnvironment(): void {
  if (!IS_DATA_DIRECTORY_ISOLATED) return;
  ...
  if (IS_PERFORMANCE_BUILD) {
    ...
    document.documentElement.setAttribute("data-development-build", "true");
  }
}
