...
import { Building2, ChevronDown, ChevronUp, Gamepad2, Heart, Layers, Sparkles, Star, User } from "lucide-react";
import { Cell, Pie, PieChart, ResponsiveContainer, Tooltip } from "recharts";
import { cn } from "../../../lib/utils_ui";
import type { StatItem } from "../../../lib/stats-logic";
import { StatsWidgetShell } from "../StatsWidgetShell";
import { STATS_WIDGET_META, type StatsWidgetDisplayMode } from "../stats-config";

type TopListKind = "platforms" | "franchises" | "studios" | "authors" | "actresses";
...
interface TopListsWidgetProps {
  items: Record<TopListKind, StatItem[]>;
  displayMode: StatsWidgetDisplayMode;
}

const TAB_CONFIG: TopListTab[] = [
  { kind: "platforms", label: "Platforms", icon: <Gamepad2 size={15} />, accentColor: "blue" },
  { kind: "franchises", label: "Franchises", icon: <Sparkles size={15} />, accentColor: "cyan" },
  { kind: "studios", label: "Studios", icon: <Building2 size={15} />, accentColor: "purple" },
  { kind: "authors", label: "Authors", icon: <User size={15} />, accentColor: "green" },
  { kind: "actresses", label: "Actresses", icon: <Heart size={15} />, accentColor: "pink" },
];

const COLOR_MAP: Record<
  TopListTab["accentColor"],
...
const DONUT_PALETTE_MAP: Record<TopListTab["accentColor"], string[]> = {
...
export function TopListsWidget({ items, displayMode }: TopListsWidgetProps) {
  const meta = STATS_WIDGET_META["top-lists"];
  const [activeKind, setActiveKind] = useState<TopListKind>("platforms");

  useEffect(() => {
    if (!items[activeKind] || items[activeKind].length === 0) {
      const firstAvailable = TAB_CONFIG.find((tab) => items[tab.kind] && items[tab.kind].length > 0);
      if (firstAvailable) {
        setActiveKind(firstAvailable.kind);
      }
    }
  }, [items, activeKind]);

  const availableTabs = useMemo(
    () => TAB_CONFIG.filter((tab) => (items[tab.kind] ?? []).length > 0),
    [items]
  );
  const activeTab = TAB_CONFIG.find((tab) => tab.kind === activeKind) ?? availableTabs[0];
  const activeItems = activeTab ? items[activeTab.kind] ?? [] : [];

  const totalAvailable = availableTabs.length;
  const colors = activeTab ? COLOR_MAP[activeTab.accentColor] : COLOR_MAP.blue;
  const donutPalette = activeTab ? DONUT_PALETTE_MAP[activeTab.accentColor] : DONUT_PALETTE_MAP.blue;
  const totalCount = activeItems.reduce((sum, item) => sum + item.count, 0);
  const maxCount = activeItems.length > 0 ? activeItems[0].count : 1;
  const storageKey = activeKind;

  const [isExpanded, setIsExpanded] = useState(
    () => localStorage.getItem(`stats-section-${storageKey}`) === "true"
  );
  ...
  const displayItems = isExpanded ? activeItems : previewItems;
  const hasMore = activeItems.length > 3;
  const donutItems = buildDonutItems(activeItems);
  const effectiveDisplayMode =
    displayMode === "donut" && donutItems.length >= 1 ? "donut" : "bars";
  ...
  return (
    <StatsWidgetShell
      ...
      title={meta.title}
      icon={<Layers size={20} />}
      subtitle={`${totalAvailable} ${totalAvailable === 1 ? "category" : "categories"} · ${activeTab?.label ?? ""}`}
      heightPreset={meta.heightPreset}
      fillBody
      badge={
        <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-xs font-medium text-gray-300">
          {totalCount}
        </span>
      }
      action={
      ...
      onHeaderClick={
        effectiveDisplayMode === "bars" ? () => setIsExpanded((current) => !current) : undefined
      }
      ...
      emptyState={
        <div className="flex h-full min-h-[220px] items-center justify-center rounded-xl border border-dashed border-white/10 bg-black/10 px-4 py-8 text-center">
          <p className="text-sm text-gray-400">No platform, franchise, studio, author, or actress data matched the current filters.</p>
        </div>
      }
    >
      <div className="flex min-h-0 flex-1 flex-col">
        {totalAvailable > 1 ? (
          <div className="mb-4 flex flex-wrap gap-1.5">
            {availableTabs.map((tab) => {
              ...
              const tabColors = COLOR_MAP[tab.accentColor];
              const tabCount = (items[tab.kind] ?? []).reduce((sum, item) => sum + item.count, 0);
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? cn(tabColors.border, tabColors.bg, tabColors.text)
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      isActive ? cn(tabColors.bg, tabColors.text) : "bg-white/10 text-gray-400"
                    )}
                  >
                    {tabCount}
                  </span>
        ...
        {effectiveDisplayMode === "donut" ? (
              ...
              <div className="relative mx-auto h-44 w-full max-w-[220px]">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip cursor={false} content={<BreakdownDonutTooltip totalCount={totalCount} />} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="custom-scrollbar flex min-h-0 flex-1 flex-col space-y-2 overflow-y-auto pr-1">
              ...
              {!isExpanded && hasMore ? (
                <div className="mt-auto pt-3 text-center">
                  <span className="text-xs text-gray-500">+{activeItems.length - 3} more • Click to expand</span>
                </div>
              ) : null}
            </div>
          </div>
