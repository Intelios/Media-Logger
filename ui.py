...
class AppUI:
    ...
    def show_import_summary_and_refresh(self, message, had_errors):
        ...
        if self.stats_view_instance and self.stats_view_instance.stats_year_filter.current:
            current_stats_filter = list(self.stats_view_instance.stats_year_filter.current.selected)[0] if self.stats_view_instance.stats_year_filter.current.selected else "All Time"
    ...
    def open_add_jav_dialog(self, e=None, initial_data=None):
        ...
        def save_new_jav(e):
            ...
            self.refresh_current_view()
            # Refresh stats if stats view is initialized
            if self.stats_view_instance:
                current_stats_filter = list(self.stats_view_instance.stats_year_filter.current.selected)[0] if self.stats_view_instance.stats_year_filter.current and self.stats_view_instance.stats_year_filter.current.selected else "All Time"
                self.page.run_thread(self.calculate_and_update_stats_display, current_stats_filter)
    ...
    def calculate_and_update_stats_display(self, filter_year="All Time"):
        """Wrapper method for stats calculation - delegates to StatsView."""
        if self.stats_view_instance:
            self.stats_view_instance.calculate_and_update_stats_display(filter_year)
    ...
    def build_stats_view(self):
        ...
        # Create StatsView instance if not exists
        if self.stats_view_instance is None:
            self.stats_view_instance = StatsView(
                self.page,
                self.app_state,
        ...
        # Build and return the stats view
        return self.stats_view_instance.build_stats_view()
